...
use App\Models\Webadmin\SlideradsModel;
use Config\Services;
use App\Libraries\Profilelib;

class Sliderads extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $datamodel = new SlideradsModel($request);
    ...
    {
        return redirect()->to(base_url('webadmin/galeri/sliderads/data'));
    }

    public function data()
    {
        $data['title'] = 'Slider Ads';
        ...
        return view('webadmin/galeri/sliderads/index', $data);
    }
    ...
    public function add()
    {
        ...
        $response->status = 200;
        $response->message = "Permintaan diizinkan";
        $response->data = view('webadmin/galeri/sliderads/add');
        return json_encode($response);
    }
    ...
    public function edit()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->status = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('webadmin/galeri/sliderads/edit', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function delete()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                $this->_db->transBegin();
                try {
                    $this->_db->table('_tb_sliderads')->where('id', $id)->delete();

                    if ($this->_db->affectedRows() > 0) {
                        if ($current->image !== null) {
                            try {
                                $dir = FCPATH . "uploads/sliderads";
    ...
    public function addSave()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $cekData = $this->_db->table('_tb_sliderads')->where(['url' => $slug . '.html'])->get()->getRowObject();
            ...
            $this->_db->transBegin();
            try {
                $this->_db->table('_tb_sliderads')->insert($data);
            } catch (\Exception $e) {
            ...
            if ($this->_db->affectedRows() > 0) {
                ...
                $response->redirect = base_url('webadmin/galeri/sliderads/data');
                return json_encode($response);
            } else {
                unlink($dir . '/' . $newNamelampiranFile);
                $this->_db->transRollback();
    ...
    public function editSave()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $oldData =  $this->_db->table('_tb_sliderads')->where('id', $id)->get()->getRowObject();
            ...
            $this->_db->transBegin();
            try {
                $this->_db->table('_tb_sliderads')->where('id', $oldData->id)->update($data);
            } catch (\Exception $e) {
                if ($filenamelampiranFile != '') {
                    unlink($dir . '/' . $newNamelampiranFile);
                }
