...
use App\Models\Webadmin\PengadaanModel;
use Config\Services;
...
class Pengadaan extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $datamodel = new PengadaanModel($request);
        ...
        foreach ($lists as $list) {
            ...
            $row[] = $no;
            $action = '<a href="javascript:actionDetail(\'' . $list->pid . '\', \'' . str_replace("'", "", $list->judul) . '\');"><button type="button" class="btn btn-primary btn-sm btn-rounded waves-effect waves-light mr-2 mb-1">
                        <i class="bx bxs-show font-size-16 align-middle"></i></button>
                        </a>
                        <a href="javascript:actionEdit(\'' . $list->pid . '\', \'' . str_replace("'", "", $list->judul) . '\');"><button type="button" class="btn btn-secondary btn-sm btn-rounded waves-effect waves-light mr-2 mb-1">
    ...
    {
        return redirect()->to(base_url('webadmin/informasi/pengadaan/data'));
    }
    ...
    public function data()
    {
        ...
        return view('webadmin/informasi/pengadaan/index', $data);
    }
    ...
    public function add()
    {
        ...
        $data['kategoris'] = $this->_db->table('_tb_kategori_pengadaan')->orderBy('kategori', 'asc')->get()->getResult();

        $response = new \stdClass;
        $response->status = 200;
        $response->message = "Permintaan diizinkan";
        $response->data = view('webadmin/informasi/pengadaan/add', $data);
        return json_encode($response);
    }
    ...
    public function edit()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('_tb_pengadaan')
            ...
            if ($current) {
                ...
                $response->status = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('webadmin/informasi/pengadaan/edit', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('_tb_pengadaan a')
                ->select("a.*, b.kategori")
                ->join('_tb_kategori_pengadaan b', 'a.k_id = b.kid', 'left')
                ->where('pid', $id)->get()->getRowObject();
            ...
            if ($current) {
                ...
                $response->status = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('webadmin/informasi/pengadaan/detail', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function addSave()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $cekData = $this->_db->table('_tb_pengadaan')->where(['url' => $slug . '.html'])->get()->getRowObject();
    ...
    public function editSave()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $oldData =  $this->_db->table('_tb_pengadaan')->where('pid', $id)->get()->getRowObject();
    ...
    public function uploadImage()
    {
        ...
        if ($validated) {
            ...
            $filesNamelampiran = $lampiran->getName();
            $newNamelampiran = _create_name_foto($filesNamelampiran);
            $writePath = FCPATH . "uploads/pengadaan/widget";

            if ($lampiran->isValid() && !$lampiran->hasMoved()) {
                $lampiran->move($writePath, $newNamelampiran);
                $data = [
                    "uploaded" => true,
                    "url" => base_url('uploads/pengadaan/widget/' . $newNamelampiran),
                ];
            } else {
                $data = [
                    "uploaded" => false,
