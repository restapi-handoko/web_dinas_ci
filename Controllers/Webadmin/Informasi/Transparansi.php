...
use App\Models\Webadmin\TransparansiModel;
use Config\Services;
...
class Transparansi extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $datamodel = new TransparansiModel($request);
        ...
        foreach ($lists as $list) {
            ...
            if ($list->image !== null) {
                $image = '<img alt="Image placeholder" src="' . base_url() . '/uploads/transparansi/' . $list->image . '" width="80px" height="50px">';
            } else {
                $image = "-";
            }
            if ($list->lampiran !== null) {
                $lampiran = '<a target="_blank" href="' . base_url() . '/uploads/transparansi/' . $list->lampiran . '" class="badge badge-pill badge-soft-success">Lampiran</a>';
            } else {
                $lampiran = "-";
            }
            $row[] = $action;
    ...
    {
        return redirect()->to(base_url('webadmin/informasi/transparansi/data'));
    }

    public function data()
    {
        $data['title'] = 'Transparansi';
        ...
        return view('webadmin/informasi/transparansi/index', $data);
    }
    ...
    public function add()
    {
        ...
        $response->status = 200;
        $response->message = "Permintaan diizinkan";
        $response->data = view('webadmin/informasi/transparansi/add');
        return json_encode($response);
    }
    ...
    public function edit()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('_tb_transparansi')
            ...
            if ($current) {
                ...
                $response->status = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('webadmin/informasi/transparansi/edit', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->status = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('webadmin/informasi/transparansi/detail', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function delete()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                $this->_db->transBegin();
                try {
                    $this->_db->table('_tb_transparansi')->where('id', $id)->delete();

                    if ($this->_db->affectedRows() > 0) {
                        if ($current->image !== null) {
                            try {
                                $dir = FCPATH . "uploads/transparansi";
    ...
    public function addSave()
    {
        ...
        $rules = [
            'judul' => [
                'rules' => 'required|trim',
                'errors' => [
                    'required' => 'Judul transparansi tidak boleh kosong. ',
                ]
            ],
            'isi' => [
                'rules' => 'required',
                'errors' => [
                    'required' => 'Deskripsi transparansi tidak boleh kosong. ',
                ]
            ],
            'status' => [
                'rules' => 'required',
        ...
        if ($filenamelampiran != '') {
            $lampiranVal = [
                '_file' => [
                    ...
                    'errors' => [
                        'uploaded' => 'Pilih gambar transparansi terlebih dahulu. ',
                        'max_size' => 'Ukuran gambar transparansi terlalu besar. ',
        ...
        if ($filenamelampiranFile != '') {
            $lampiranValFile = [
                '_file_lampiran' => [
                    ...
                    'errors' => [
                        'uploaded' => 'Pilih file transparansi terlebih dahulu. ',
                        'max_size' => 'Ukuran file transparansi terlalu besar. ',
        ...
        if (!$this->validate($rules)) {
            ...
            $cekData = $this->_db->table('_tb_transparansi')->where(['url' => $slug . '.html'])->get()->getRowObject();
            ...
            $this->_db->transBegin();
            try {
                $this->_db->table('_tb_transparansi')->insert($data);
            } catch (\Exception $e) {
                if ($filenamelampiran != '') {
            ...
            if ($this->_db->affectedRows() > 0) {
                ...
                $response->redirect = base_url('webadmin/informasi/transparansi/data');
                return json_encode($response);
            } else {
                if ($filenamelampiran != '') {
                    unlink($dir . '/' . $newNamelampiran);
    ...
    public function editSave()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $oldData =  $this->_db->table('_tb_transparansi')->where('id', $id)->get()->getRowObject();
            ...
            $this->_db->transBegin();
            try {
                $this->_db->table('_tb_transparansi')->where('id', $oldData->id)->update($data);
            } catch (\Exception $e) {
                if ($filenamelampiran != '') {
                    unlink($dir . '/' . $newNamelampiran);
                }
    ...
    public function uploadImage()
    {
        ...
        if ($validated) {
            ...
            $filesNamelampiran = $lampiran->getName();
            $newNamelampiran = _create_name_foto($filesNamelampiran);
            $writePath = FCPATH . "uploads/transparansi/widget";

            if ($lampiran->isValid() && !$lampiran->hasMoved()) {
                $lampiran->move($writePath, $newNamelampiran);
                $data = [
                    "uploaded" => true,
                    "url" => base_url('uploads/transparansi/widget/' . $newNamelampiran),
                ];
            } else {
                $data = [
                    "uploaded" => false,
