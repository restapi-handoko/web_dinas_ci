...
class Produkhukum extends BaseController
{
    ...
    public function index()
    {
        ...
        $countBerita = $this->_db->table('_tb_regulasi')->where('status', 1)->countAllResults();
        $perPage = 10;
        ...
        $data['dataProdukHukum'] = $this->_db->table('_tb_regulasi a')
            ->select("a.*, b.kategori")
            ->join("_tb_kategori_regulasi b", "b.kid = a.k_id")
            ->where('a.status', 1)
            ->orderBy('a.created_at', 'DESC')
        ...
        return view('web/produkhukum/index', $data);
    }
    ...
    public function detail($url)
    {
        ...
        $data['dataWidgetPengumuman'] = $this->_db->table('_tb_pengumuman a')
            ...
            ->get()
            ->getResult();
        return view('web/produkhukum/detail', $data);
    }
}
