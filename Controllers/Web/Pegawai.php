...
class Pegawai extends BaseController
{
    ...
    public function index()
    {
        ...
        $countBerita = $this->_db->table('_tb_pegawai')->where('status', 1)->countAllResults();
        $perPage = 10;
        ...
        $data['dataPegawai'] = $this->_db->table('_tb_pegawai a')
            ->select("a.*, b.jabatan")
            ->join("_tb_jabatan b", "b.jid = a.j_id")
            ->where('a.status', 1)
            ->orderBy('a.created_at', 'DESC')
        ...
        return view('web/pegawai/index', $data);
    }
}
