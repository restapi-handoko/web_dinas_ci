...
class Transparansi extends BaseController
{
    ...
    {
        $data['title'] = 'Transparansi & Inovasi';
        $data['admin'] = false;
        ...
        $countBerita = $this->_db->table('_tb_transparansi')->where('status', 1)->countAllResults();
        $perPage = 10;
        ...
        $data['dataTransparansi'] = $this->_db->table('_tb_transparansi a')
            ->select("a.*")
            ->where('a.status', 1)
            ->orderBy('a.created_at', 'DESC')
        ...
        return view('web/transparansi/index', $data);
    }

    public function detail($url)
    {
        // Query database
        $data['transparansi'] = $this->_db->table('_tb_transparansi a')
            ->select("a.*")
            ->where('a.url', $url)
            ->where('a.status', 1)
            ->get()
        ...
        if (!$data['transparansi']) {
            return view('404');
        }
        $data['title'] = 'Detail Transparansi & Inovasi';
        $data['admin'] = false;
        ...
        $data['dataWidgetPengumuman'] = $this->_db->table('_tb_pengumuman a')
            ...
            ->get()
            ->getResult();
        return view('web/transparansi/detail', $data);
    }
}
